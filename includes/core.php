...

{
	...
	$rg_stores_hook_suffixes = array(
		...
		'revglue-stores_page_revglue-broadbands',
...

	{
		...
		wp_register_style( 'rg-store-confirm', RGSTORE__PLUGIN_URL . 'admin/css/jquery-confirm.css' );

		wp_enqueue_style( 'rg-store-confirm' );

		wp_register_style( 'rg-store-confirm-bundled', RGSTORE__PLUGIN_URL . 'admin/css/bundled.css' );

		wp_enqueue_style( 'rg-store-confirm-bundled' );

		wp_register_style( 'rg-store-main', RGSTORE__PLUGIN_URL . 'admin/css/admin_style.css' );

		wp_enqueue_style( 'rg-store-main' );

		

		wp_register_style( 'rg-store-checkbox', RGSTORE__PLUGIN_URL . 'admin/css/iphone_style.css' );

		wp_enqueue_style( 'rg-store-checkbox' );

		wp_register_style( 'rg-store-datatables', RGSTORE__PLUGIN_URL . 'admin/css/jquery.dataTables.css' );

		wp_enqueue_style( 'rg-store-datatables' );

		wp_register_style( 'rg-store-fontawesome', RGSTORE__PLUGIN_URL . 'admin/css/font-awesome.css' );

		wp_enqueue_style( 'rg-store-fontawesome' );



		wp_register_style( 'rg-store-bootstrap-min', RGSTORE__PLUGIN_URL . 'admin/css/bootstrap.min.css' );

		wp_enqueue_style( 'rg-store-bootstrap-min' );

		wp_register_style( 'revglue-jscrollpane', RGSTORE__PLUGIN_URL . 'admin/css/jquery.jscrollpane.css' );

	wp_enqueue_style( 'revglue-jscrollpane' ); 



		// Enqueue Admin Scripts

		wp_register_script( 'rg-store-datatables', RGSTORE__PLUGIN_URL . 'admin/js/jquery.dataTables.js', array ( 'jquery' ) );

		wp_enqueue_script( 'rg-store-datatables' );

		wp_register_script( 'rg-store-unveil', RGSTORE__PLUGIN_URL . 'admin/js/jquery.unveil.js', array ( 'jquery' ) );

		wp_enqueue_script( 'rg-store-unveil' );

		wp_register_script( 'rg-store-checkbox', RGSTORE__PLUGIN_URL . 'admin/js/iphone-style-checkboxes.js', array ( 'jquery' ) );

		wp_enqueue_script( 'rg-store-checkbox' );

		wp_register_script( 'rg-store-confirm', RGSTORE__PLUGIN_URL . 'admin/js/jquery-confirm.js', array ( 'jquery' ) );

		wp_enqueue_script( 'rg-store-confirm' );

		wp_register_script( 'revglue-jscrollpane', RGSTORE__PLUGIN_URL . 'admin/js/jquery.jscrollpane.min.js', array ( 'jquery' ) );

 	wp_enqueue_script( 'revglue-jscrollpane' );

		wp_register_script( 'revglue-mousewheel',RGSTORE__PLUGIN_URL . 'admin/js/jquery.mousewheel.js', array ( 'jquery' ) );

 		wp_enqueue_script( 'revglue-mousewheel' );


		wp_register_script( 'revglue-notify', RGSTORE__PLUGIN_URL . 'admin/js/notify.js', array ( 'jquery' ) );

 		wp_enqueue_script( 'revglue-notify' );

		wp_register_script( 'rg-store-main', RGSTORE__PLUGIN_URL . 'admin/js/main.js', array ( 'jquery', 'jquery-form' ) );

		wp_enqueue_script( 'rg-store-main' );



		wp_register_script( 'rg-store-bootstrap-min', RGSTORE__PLUGIN_URL . 'admin/js/bootstrap.min.js', array ( 'jquery', 'jquery-form' ) );

		wp_enqueue_script( 'rg-store-bootstrap-min' );
...

{
	...
	add_menu_page('RevGlue Stores', 'RevGlue Stores', 'manage_options', 'revglue-dashboard', 'rg_stores_main_page', RGSTORE__PLUGIN_URL .'admin/images/menuicon.png' );

	

	add_submenu_page('revglue-dashboard', 'Dashboard', 'Dashboard', 'manage_options', 'revglue-dashboard', 'rg_stores_main_page');

	add_submenu_page('revglue-dashboard', 'Import BB Data', 'Import BB Data', 'manage_options', 'revglue-import-stores', 'rg_stores_store_import_page');

	add_submenu_page('revglue-dashboard', 'Stores', 'Stores', 'manage_options', 'revglue-stores', 'rg_stores_listing_page');

	add_submenu_page('revglue-dashboard', 'Broadband & TV', 'Broadband & TV', 'manage_options', 'revglue-broadbands', 'rg_broadbands_listing_page');
...

{
...

	{
		...
		include( RGSTORE__PLUGIN_DIR . 'includes/auto-import-data.php');
		...
		<td style="text-align:left;">
			...
			<div class="revglue-banner-thumb rg_store_icon_thumb_<?php echo  $category_object->rg_category_id ?>">
...

				{
					...
					?><a id="<?php echo  $category_object->rg_category_id ?>" data-type="image_url" class="rg_category_delete_icons" href="javascript;"><i class="fa fa-times" aria-hidden="true"></i></a>
		...
		<td style="text-align:left;">
			...
			<div class="revglue-banner-thumb rg_store_image_thumb_<?php echo  $category_object->rg_category_id ?>">
...

				{
					...
					?><a id="<?php echo  $category_object->rg_category_id ?>" data-type="icon_url" class="rg_category_delete_images" href="javascript;"><i class="fa fa-times" aria-hidden="true"></i></a>
...
function revglue_broad_user_subscription_id() {
...
add_action( 'admin_notices', 'revglue_broad_user_subscription_id' );
...
function revglue_broad_remove_default_widgets(){
...
add_action('wp_dashboard_setup', 'revglue_broad_remove_default_widgets');
